...
export default function CustomizedDialogs({ isOpen, handleCloseFromMain }) {
  ...
  const handleScan = async (data) => {
    if (data) {
      ...
      axios
        .get(
          `https://esystems.cdl.lk/backend/BizTrack/EWODetails/GetEWODetails?EWONo=${ewdNo}`
        )
        .then((response) => {
          ...
          if (response.data.StatusCode === 200) {
            ...
            if (!res) {
              ...
              if (res.issued_by != "") {
                ...
                axios
                  .post(
                    `https://esystems.cdl.lk/backend/BizTrack/EWODetails/RecieveEWODetails`,
                    requestBody
                  )
                  .then((response) => {
  ...
  const handleSaveChanges = async () => {
    ...
    axios
      .post(
        `https://esystems.cdl.lk/backend/BizTrack/EWODetails/SendEWODetails`,
        data
      )
      .then((response) => {
        ...
        axios
          .get(
            `https://esystems.cdl.lk/backend/BizTrack/EWODetails/GetEWODetails?EWONo=${responseData.ewo_no}`
          )
          .then((res) => {
            setResponseData(res);
            console.log("Done");
